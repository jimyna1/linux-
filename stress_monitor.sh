...
cleanup_expired_time_slots() {
  ...
  #  {
   ...
   # }
    ...
    local cutoff_date
      cutoff_date=$(date -d "$SLOT_EXPIRE_DAYS days ago" +%F)
...
get_cpu_usage() {
    cpu_idle=$(top -bn1 | sed -n '2s/.* $[0-9]\+\(\.[0-9]\+$\?%\) id.*/\1/p' 2>/dev/null)
    if [ -z "$cpu_idle" ]; then
        echo 0
    else
        # 计算CPU使用率（100% - 空闲率），并使用printf进行格式化输出
        printf "%.0f\n" "$(echo "100 - ${cpu_idle%\%}" | bc)"
    fi
}
get_mem_usage(){
    # MemTotal 总内存，MemAvailable可用内存，内核标准接口，不受free版本影响
    ...
    # awk计算百分比，整数输出
    awk -v total="${mem_total}" -v avail="${mem_available}" 'BEGIN{printf "%.0f", (total - avail)/total *100}'
}
# 获取本月已经执行天数
get_month_exec_days(){
    # 统计日志文件中本月日期的记录行数，模拟本月已执行天数
    local current_month
